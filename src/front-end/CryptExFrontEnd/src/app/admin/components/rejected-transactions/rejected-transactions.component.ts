...
export class RejectedTransactionsComponent implements OnInit, OnDestroy {
  ...
  updatePagination(): void {
    if (!this.rejectedTransactions || this.rejectedTransactions.length === 0) {
      this.totalPages = 1;
      this.currentPage = 1;
    ...
    this.totalPages = Math.ceil(this.rejectedTransactions.length / this.pageSize);
    ...
    const endIndex = Math.min(startIndex + this.pageSize, this.rejectedTransactions.length);
    
    // Create a deep copy to avoid reference issues
    this.paginatedTransactions = this.rejectedTransactions.slice(startIndex, endIndex).map(t => ({...t}));
